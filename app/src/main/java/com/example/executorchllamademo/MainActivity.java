...
public class MainActivity extends AppCompatActivity implements Runnable, LlmCallback {
  ...
  private void setLocalModel(
    ...
    try {
      int modelCategory = ModelUtils.getModelCategory(
          mCurrentSettingsFields.getModelType(), mCurrentSettingsFields.getBackendType());
      ETLogging.getInstance().log("Creating LlmModule with model category: " + modelCategory);
      ...
      if (dataPath.isEmpty()) {
        ...
        ETLogging.getInstance().log("Passing dataPath to LlmModule: " + dataPath);
        mModule =
            new LlmModule(
                modelCategory,
                modelPath,
      ...
      ETLogging.getInstance().log("LlmModule created successfully, calling load()...");
    } catch (Exception e) {
      ETLogging.getInstance().log("EXCEPTION during LlmModule creation: " + e.getMessage());
      ETLogging.getInstance().log("Exception type: " + e.getClass().getName());
      e.printStackTrace();
      return;
    }
    
    ETLogging.getInstance().log("About to call mModule.load()...");
    int loadResult = -999; // sentinel value
    try {
      loadResult = mModule.load();
      ETLogging.getInstance().log("mModule.load() returned: " + loadResult);
    } catch (Exception e) {
      ETLogging.getInstance().log("EXCEPTION during load(): " + e.getMessage());
      e.printStackTrace();
    }
    ...
    if (loadResult != 0) {
      ...
      String errorDescription = getErrorDescription(loadResult);
      modelInfo = "*Model could not load (Error Code: " + loadResult + ")*\n" + errorDescription + "\n";
      modelLoadError = "Error: " + errorDescription + "\n";
      loadDuration = 0;
      
      ETLogging.getInstance().log("Load FAILED with error code: " + loadResult);
      ETLogging.getInstance().log("Error description: " + errorDescription);
      ETLogging.getInstance().log("Model path: " + modelPath);
      ETLogging.getInstance().log("Tokenizer path: " + tokenizerPath);
      ETLogging.getInstance().log("Data path: " + dataPath);
      ETLogging.getInstance().log("Backend: " + mCurrentSettingsFields.getBackendType().toString());
      ETLogging.getInstance().log("Model Type: " + mCurrentSettingsFields.getModelType().toString());
      
      AlertDialog.Builder builder = new AlertDialog.Builder(this);
      builder.setTitle("Load failed: " + loadResult);
      builder.setMessage(errorDescription + "\n\nPlease check logcat for detailed logs with tag [ET-*]");
      runOnUiThread(
          () -> {
  ...
  private String getErrorDescription(int errorCode) {
    // ExecuTorch error codes mapping
    switch (errorCode) {
      case 0x00: return "Success (shouldn't see this as error)";
      case 0x01: return "Internal error occurred";
      case 0x02: return "Invalid state";
      case 0x03: return "End of method";
      case 0x10: return "Not supported";
      case 0x11: return "Not implemented";
      case 0x12: return "Invalid argument - check model path, tokenizer path, or configuration";
      case 0x13: return "Invalid type";
      case 0x14: return "Operator missing";
      case 0x20: return "Not found - model or tokenizer file may be missing";
      case 0x21: return "Memory allocation failed - insufficient memory";
      case 0x22: return "Access failed - check file permissions";
      case 0x23: return "Invalid program - model file may be corrupted";
      default: 
        if (errorCode == 18) {
          return "Invalid program (0x12) - Model file format error or incompatible version.\n" +
                 "Check:\n" +
                 "• Model file is valid .pte format\n" +
                 "• Model was compiled for QNN backend\n" +
                 "• Tokenizer file exists and is valid\n" +
                 "• All required library files are present\n" +
                 "• Check logcat for [ET-*] tags for details";
        }
        return "Unknown error code: " + errorCode + " (0x" + Integer.toHexString(errorCode) + ")";
    }
  }
  ...
  private String buildQnnConfigString(ModelType modelType) {
    // Build configuration string for QNN backend
    // Format: decoder_model_version:qwen3;kv_updater:SmartMask;eval_mode:1
    String decoderModelVersion = "llama3"; // default
    
    switch (modelType) {
      case QWEN_3:
        decoderModelVersion = "qwen3";
        break;
      case LLAMA_3:
        decoderModelVersion = "llama3";
        break;
      case GEMMA_3:
        decoderModelVersion = "gemma3";
        break;
      case LLAMA_GUARD_3:
        decoderModelVersion = "llama3";
        break;
    ...
    return "decoder_model_version:" + decoderModelVersion + ";kv_updater:SmartMask;eval_mode:1";
  }
  ...
  private void checkForUpdateAndReloadModel(SettingsFields updatedSettingsFields) {
    ...
    if (!modelPath.isEmpty() && !tokenizerPath.isEmpty()) {
      if (updatedSettingsFields.getIsLoadModel()
        ...
        // For QNN backend, build configuration string
        String finalDataPath = dataPath;
        if (updatedSettingsFields.getBackendType() == BackendType.QUALCOMM) {
          String qnnConfig = buildQnnConfigString(updatedSettingsFields.getModelType());
          ETLogging.getInstance().log("QNN Config: " + qnnConfig);
          finalDataPath = qnnConfig;
        }
